...
const Update=({navigation,route})=>{
    ...
    const id=(JSON.stringify(pk)).replace(/\"/g,"")
    ...
    useEffect(()=>{
        ...
        const get=async ()=>{
            ...
            setpython(response.data.skill_python)
            setjavascript(response.data.skill_javascript)
            setjava(response.data.skill_java)
            setLeave(response.data.leave_type)
            console.log(response.data)
            console.log(data)//empty object
            console.log('i am end update')
...
// console.log(data)
   ...
   const update=async ()=>{
    try{
        ...
        const response=await axios.put(`http://10.0.2.2:8000/leave/leavespecific/${id}`,
                                                                {reason:reason,
        ...
        setData(response.data)
        console.log(response.data)
        console.log('I am in ultimate update')
    ...
    <ScrollView style={{backgroundColor:'white'}} showsVerticalScrollIndicator={false} >
        <View style={{margin:20}}>
            ...
            <View style={{flexDirection:'row',top:-55}}>
                ...
                >
                    <Icons name='save' size={28} color='white'/>
                    <Text style={{ fontWeight: 'bold', fontSize: 22,color:'white',marginLeft:6 }}>Update</Text>
               </Button>
               <Button
                title="Submit"
                loading={false}
                ...
                >
                    ...
                    <Text style={{ fontWeight: 'bold', fontSize: 22,color:'white',marginLeft:6 }}>Cancel</Text>
               </Button>
            </View>    
            ...
            {/* <View>
                ...
                <Text>{data.leave_type}</Text>
                <Text>{data.reason}</Text>
                <Text>{data.start_data}</Text>
                <Text>{data.end_date}</Text>
            </View> */}
            </View>)
            :
            (
                <Text style={{ fontWeight: 'bold', fontSize: 30,color:'black',margin:20 }}>loading...</Text>
            )
